...
class ExchangeService(BaseService):
  def __init__(self):
    ...
    self.rpc[0x01] = self.CreateOrderBook
    self.rpc[0x02] = self.PlaceOfferOnOrderBook
    self.rpc[0x03] = self.PlaceOfferCreateOrderBookIfNeeded
    self.rpc[0x04] = self.PlaceBidOnOrderBook
    self.rpc[0x05] = self.PlaceBidCreateOrderBookIfNeeded
    self.rpc[0x06] = self.QueryOffersByOrderBook
    self.rpc[0x07] = self.QueryBidsByOrderBook
    self.rpc[0x08] = self.QueryOffersByAccountForItem
    self.rpc[0x09] = self.QueryBidsByAccountForItem
    self.rpc[0x0a] = self.QueryOrderBooksSummary
    self.rpc[0x0b] = self.QuerySettlementsByOrderBook
    self.rpc[0x0c] = self.ReportAuthorize
    self.rpc[0x0d] = self.ReportSettle
    self.rpc[0x0e] = self.ReportCancel
    self.rpc[0x0f] = self.SubscribeOrderBookStatusChange
    self.rpc[0x10] = self.UnsubscribeOrderBookStatusChange
    self.rpc[0x11] = self.SubscribeOrderStatusChange
    self.rpc[0x12] = self.UnsubscribeOrderStatusChange
    self.rpc[0x13] = self.GetPaymentMethods
    self.rpc[0x14] = self.ClaimBidItem
    self.rpc[0x15] = self.ClaimBidMoney
    self.rpc[0x16] = self.ClaimOfferItem
    self.rpc[0x17] = self.ClaimOfferMoney
    self.rpc[0x18] = self.CancelBid
    self.rpc[0x19] = self.CancelOffer
    self.rpc[0x1a] = self.GetConfiguration
    self.rpc[0x1b] = self.GetBidFeeEstimation
    self.rpc[0x1c] = self.GetOfferFeeEstimation
